...
const useScifiSound = () => {
  const playTone = useCallback((freq = 600, type = "sine", duration = 0.1) => {
    try {
  ...
  return {
    hover: () => playTone(300, "triangle", 0.05),
...
const ProjectCard = ({ project }) => {
  ...
  const rotateX = useMotionTemplate`${mouseY.get() / 25}deg`; // Reduced tilt for stability
  ...
  return (
    <motion.div
      ...
      // ✅ FIX: Reduced min-height to 420px (was 480px) to make it look wider/shorter
    ...
    >
      ...
      <div className="relative flex flex-col h-full overflow-hidden rounded-2xl border border-slate-800 bg-[#0f172a]/95 backdrop-blur-xl shadow-2xl transition-colors duration-300 group-hover:border-slate-700/80">
        ...
        {/* --- IMAGE HEADER (Wider Ratio) --- */}
        {/* ✅ FIX: Reduced height to h-44/h-48 to emphasize width */}
