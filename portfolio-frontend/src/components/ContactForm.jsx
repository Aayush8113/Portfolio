...
// --- 🔊 SOUND SYSTEM (Singleton Service) ---
const AudioService = {
  ctx: null,
  init() {
  ...
  playTone(freq = 1000, type = "sine", duration = 0.1, vol = 0.1) {
    try {
      const ctx = this.init();
      if (!ctx) return;
      ...
      // Audio autoplay policy or system mute
    }
  },
};

// --- 🪝 CUSTOM HOOK: Sound Effects ---
const useScifiSound = () => {
  return useMemo(
    () => ({
      playHover: () => AudioService.playTone(800, "triangle", 0.05, 0.05),
      playType: () =>
        AudioService.playTone(600 + Math.random() * 200, "sine", 0.05, 0.05),
      playError: () => {
        AudioService.playTone(150, "sawtooth", 0.2, 0.2);
        setTimeout(() => AudioService.playTone(100, "sawtooth", 0.2, 0.2), 100);
      },
      playSuccess: () => {
        AudioService.playTone(800, "sine", 0.1, 0.1);
        setTimeout(() => AudioService.playTone(1200, "sine", 0.2, 0.1), 150);
        setTimeout(() => AudioService.playTone(2000, "square", 0.4, 0.1), 300);
      },
    }),
    [],
  );
...
// Extracted to prevent re-renders of the entire form on every keystroke
const FormInput = ({
  name,
  icon: Icon,
  placeholder,
  ...
  const isValid = touched && !error && (name === "phone" ? true : value !== "");
...
const ContactForm = () => {
  ...
  const validate = (name, value) => {
    ...
    if (!rule) return null;
    if (!value || value.trim() === "")
      return rule.optional ? null : "MISSING DATA";
  ...
  const handleSubmit = async (e) => {
    ...
    // 3. Concurrent Execution: Animation + API Call
    // We run the API call alongside the animation to mask latency (Senior Optimization)
    try {
      ...
      for (const step of steps) {
        setUploadStep(step);
        playHover();
        await new Promise((r) => setTimeout(r, 400)); // Minimum visual delay for effect
      }
  ...
  // Calculate "Security Level" (Completion %)
  const validFields = Object.keys(formData).filter(
    (k) => k !== "phone" && !validate(k, formData[k]),
  ).length;
  const securityLevel = Math.min((validFields / 3) * 100, 100);
            ...
            {status.success ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                ...
                <FormInput
                  name="phone"
                  icon={Phone}
                  placeholder="UPLINK CODE (OPTIONAL)"
                  type="tel"
